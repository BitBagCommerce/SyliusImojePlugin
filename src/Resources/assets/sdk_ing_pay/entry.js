export const performAction = async (isAfterPayment = false) => {
    ...
    const isIngPay = isAfterPayment
        ? ''
        : document.querySelector('[data-bb-is-ing-pay-method]').dataset.bbIsIngPayMethod;

    if ('card' === paymentMethod && 'ingPayPaymentMethods' === isIngPay || isAfterPayment) {
        ...
        try {
            ...
            script.id = 'ing-pay-widget__script';
...
const widgetIngPay = document.querySelector('.js-widget-ing-pay-action');
const turnOnListener = () => {
    if (widgetIngPay) {
        widgetIngPay.addEventListener('click', e => {
            e.preventDefault();
            performAction();
        });
    }
